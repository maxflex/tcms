...
(function() {
  ...
  angular.module('Egecms').controller('MobileMenuIndex', function($rootScope, $scope, $timeout, $http, Menu) {
    ...
    $scope.hideSection = function(item) {
      item.is_hidden = !item.is_hidden;
      return $http.put('api/menu-sections/' + item.id, {
...
(function() {
  angular.module('Egecms').controller('VideosIndex', function($scope, $attrs, $timeout, IndexService, Video, FolderService) {
    bindArguments($scope, arguments);
    return angular.element(document).ready(function() {
      IndexService.init(Video, $scope.current_page, $attrs, {
        folder: $scope.folder
      });
      return FolderService.init($scope.template["class"], $scope.folder, IndexService, Video);
    });
  }).controller('VideosForm', function($scope, $attrs, $timeout, FormService, Video, Tag, FolderService) {
    bindArguments($scope, arguments);
    angular.element(document).ready(function() {
      FolderService.init($scope.template["class"]);
      FormService.init(Video, $scope.id, $scope.model);
      if (!FormService.model.id && $.cookie('video_folder_id')) {
        return FormService.model.folder_id = $.cookie('video_folder_id');
      }
    });
    return $scope.loadTags = function(text) {
      return Tag.autocomplete({
        ...
        return function() {
          return _this.methods.updateResultImage(function() {
            return $timeout(function() {
              ...
              fd.append('watermark', _this.watermark);
